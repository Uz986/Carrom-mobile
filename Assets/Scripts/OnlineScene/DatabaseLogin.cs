...
public class DatabaseLogin : MonoBehaviour {
    ...
    IEnumerator LoginUser (string _username,string _password)
    {
        ...
        if (response == "Success")
        {
            ...
            LoggedIn_DisplayUsernameText.text =  UserAccountManager.LoggedIn_playerUsername;
    ...
    {
        IEnumerator e = DCF.RegisterUser(_username, _password, "WINS:0/LOSS:0"); // << Send request to register a new user, providing submitted username and password. It also provides an initial value for the data string on the account, which is "Hello World".
        while (e.MoveNext())
        {
            yield return e.Current;
        }
